...
| `base_url` | string | **Printable form.** Not the URL to call. No userinfo and no query, and since 2026-09-13 a value that is not an http(s) address with a host — the shape `lx config set` refuses to write — is `(not an http:// or https:// address)` rather than itself, with `error` present. `""` when the block has no `base_url` key: that is a working backend, whose provider class supplies the default, and it carries no `error`. A key that is present and blank is `""` **with** `error`, because the provider sends it as written. |
