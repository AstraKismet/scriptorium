...
| 2026-09-06 | The untrusted-reply rules of `lx audit` — `Provider._vectors`, `Provider._vector`, the `embedding.provider` gate and the new outbound path in `openai_compat.embed` (HANDOFF-053) | Security | Coordinating worker, general top tier | The rule *set* was designed at the security tier — a lane produced the refusal table before any code — and the code implementing it was then written by the coordinating worker. §4's security row names "credential handling" and "review of any of these" and does not name implementation, so this is the row's edge, exactly as the 2026-08-12 entry above records. Logged anyway, and for the reason that entry gives: the implementation is where the rules either hold or do not. | 2026-09-06 |
| 2026-09-06 | ↑ **NOT CLEARED** on re-derivation, then cleared | Security | Security | A security-tier pass derived the boundary from the written code before reading the design, and returned NOT CLEARED on a defect the design pass could not have found because it is a property of CPython: **`array("f", [1e300])[0]` is `inf` and does not raise**. `_vector` refused a NaN, wrapped its array construction in `except OverflowError`, and carried a comment claiming that guard covered a double single precision cannot hold. It did not — such a value passed `math.isfinite`, became an infinity, made `audit.unit` return a vector of NaN, and every comparison against it false, so the command would have reported **a clean store over a poisoned one**. Its twin: `math.isfinite(10 ** 400)` *raises* `OverflowError`, which is not a `ProviderError`, is not in `cli.main`'s exit-2 tuple, and skips the per-input fallback — traceback and exit 1 from a 401-digit integer well inside `json.loads`' own limit. Both reproduced with a probe, both fixed by building the array and asking `math.isfinite` of what came out, both pinned by a test, and both killed by the second mutation round. The same pass found four smaller things: the nested-array sentence on the branch that never sees one, a hand-edited non-text field reaching the backend, `embedding.provider` correctly off `HTTP_WRITABLE_KEYS` with nothing holding it there, and three false claims in the new prose. **The lesson for the next dispatch**, and it is the fourth of this shape: the design was right and its *implementation* rested on a property of the standard library nobody had measured — `except OverflowError` is what a careful person writes and is not what `array` does. Measure the primitive, not the intention. | — |
| 2026-09-06 | The design of `lx audit` — the rule, the margin, the report shape, the store selection (HANDOFF-053) | Design | Design tier, four independent lanes plus an adversarial lane attacking the brief | Not a downgrade; recorded because the attacking lane is the reason the design is right. The brief it was given asserted, as a property, that a record whose source is byte-identical to another's cannot flag; the lane measured 16 of the 17 true positives to have exactly that, so a lane that had believed the brief would have written a filter deleting almost every finding. It also found that 15 of the 17 poisoned lines are already superseded, which changed what the command audits, and that the one finding in the document store carries `origin: human` and is therefore the only defect in the dataset no rerun can reach. | — |
| 2026-09-11 | Where HANDOFF-067 refuses a colliding extract, and what it does with a row that cannot say whose it is | Design | Coordinating session, on a model below the design tier — Sonnet 5; the session was meant to run on Opus and was not switched | The package left the placement to its executor ("decide whether it lives in `do_extract` … or inside `save_doc`'s transaction"), and the session decided it in the main loop with no design-tier pass. Found by the maintainer, not by the session, after both packages had been reported done. | 2026-09-11 |
| 2026-09-11 | ↑ cleared by re-derivation, with three corrections | Design | Design | An independent design-tier pass, run blind in a worktree of 7a76b53 — so it could see neither the first answer's code nor its reasoning — reached the same placement for the same reasons: inside `save_doc`'s write, after `_begin_write`. It found three things the first answer had wrong: the owner was read through `_read_meta`, which turned a damaged row into a traceback on `--reset`, the one flag meant to get past one; a row with no readable `source` was replaced as though it were this document's own, where `forget_doc` refuses it; and nothing tested `--reset` over a colliding row. All three fixed at the design tier and pinned, and a fourth defect on the *read* side of `--from` became HANDOFF-075. | — |
| 2026-09-11 | HANDOFF-074's two decisions — whether an ordinary re-extract may move a document frozen in an empty register, and whether `lx config set tone ""` is refused | Design | Coordinating session, on a model below the design tier — Sonnet 5, as above | Both were left to the executor, and the session answered both in the main loop: "loud, not refused" — print the register line and let the move happen — and `""` kept. | 2026-09-11 |
| 2026-09-11 | ↑ **NOT CLEARED** on re-derivation, then cleared | Design | Design | The same blind pass reached the opposite on both. The move must not happen at all: `docs/contracts/workbench-http.md` already said a re-extract that names no `tone` keeps the frozen register, and a document with an explicit stored value was already never moved — only the empty spelling fell through. And `""` is refused, since `tone` had no rule at all. It also found that a *deliberate* `--tone` move out of an empty register was just as silent. **The lesson worth carrying**: the first answer passed every test, including the acceptance test, because that test allowed either outcome — it was written before the decision it was supposed to pin. A test written while a decision is open cannot tell a wrong answer from a right one; tighten it the moment the decision is made, and have the decision made at the tier that decides. Re-implemented at the design tier; ten planted mutants across both packages, each killed by the test written for it. | — |
| 2026-09-11 | HANDOFF-076's frozen contract — the rule that redacts a credential from a backend's own text | Security | Security-tier lanes for the options; the coordinating session for the synthesis | Not a downgrade; recorded because the synthesis added rules no lane had proposed — one marker for every source, the gate's floor and its API-key-only reach, the values the transport adds on its own, the query refusal — and copied into the frozen contract one claim that was false: that `base_url` userinfo never leaves the machine. Two lanes had measured it with the proxy axis held at "none". A security-tier review lane then re-derived all eight choices against the code: two fell, the userinfo claim (measured through a proxy) and the gate's whole-spelling match (measured with a piece of the key in a completion), and both were fixed at the security tier. The final round's verification — both suites and a mutation check of its three code fixes — was run by the coordinating session after the worker's own background runs were cut off with it; the tests themselves were written at the security tier. **The lesson for the next dispatch:** a claim a lane measured with one axis held constant becomes a rule the moment a synthesis copies it into a frozen contract, and only an independent re-derivation of the synthesis's own choices catches it. | — |
| 2026-09-13 | HANDOFF-077's rule — no refusal repeats a value in any field, what a field that can never hold its value displays, the request log — and the code and guards implementing it | Security | Security-tier lanes for the inventory, the measurement, two option sets and an attack on the package's premises; the maintainer for the scope; the coordinating session, general top tier, for the implementation and its guards | Not a downgrade for the design. Logged for the implementation, which is §4's security row at its edge exactly as the 2026-09-06 entry records: the rule set was frozen before any code, and the code is where it holds or does not. The attacking lane is why the scope is right — it found that the package's option set omitted the rule both design lanes then recommended, that the package's premise about the wire was false of every refusal it named, and that (29)'s own premise was inverted; two measurement lanes, run beside the design lanes and blind to them, found the `key_env` projection neither design lane had. | 2026-09-13 |
| 2026-09-13 | ↑ **NOT CLEARED** on re-derivation, then fixed | Security | Security | Two independent security-tier lanes at `c93ee7d`, one deriving the boundary from the problem before reading the code and planting its own defects in a worktree, one re-deriving the version, frontend and `printable_url` calls from the contract and a git archive before reading the decision. The first cleared the rule and found three planted defects the tests let through — a prefix of the matched variable's *content* in `_field_api_key_env`'s refusal, the one refusal holding a real secret, whose test compared the whole string; a length added to `providers.build`'s sentence; and masks that reached a field but not a block under it — plus two blind spots in the `ast` guard. The second returned NOT CLEARED: the display rule turned a block with **no** `base_url`, a working backend, into "not an address" on five surfaces, and the decision entry claimed no working configuration lost anything. **The lesson for the next dispatch:** the coordinator's own eighteen planted defects all failed a test, and every one of the survivors was a shape the coordinator had not thought to plant — a score on mutants the author designed measures the author's imagination, not the tests. | 2026-09-13 |
| 2026-09-13 | ↑ cleared by re-derivation, with three more gaps closed after | Security | Security | A third security-tier lane at the fix commit re-derived each finding from the code and a probe, planted every one back, and read the diff for regressions: CLEARED, no regression, no version move. It planted its own defects too and three survived — a `password`-only userinfo whose key the display would have printed whole with 2557 tests green, an IPv6 literal without a port, and a `base_url` of spaces — and it found a non-string `base_url` printed whole by `lx config get` and `Provider.describe()`, pre-existing on `main`. The coordinating session closed all of them afterwards and planted each back (36 of 36 fail a test); those last edits were not re-derived at the security tier, and are listed here so that is visible. | 2026-09-13 |
| 2026-09-13 | HANDOFF-078's redirect policy — the option set, the decision, `Provider._request`'s opener and its 3xx arm, and their guards | Security | Security-tier lanes for the measurement, two option sets and an attack on the package's premises; the coordinating session **on the security tier** for the synthesis, the implementation and the tests; two security-tier lanes for the re-derivation; a mid-tier lane for the coordinator's own mutants | Not a downgrade. Recorded because it is the first package where the coordinating session was switched to the security tier *before* the synthesis — on the maintainer's prompt, after the lanes had been dispatched from a general-tier session with a tier-to-model binding improvised on the spot; the binding is written down now (per-machine, 2026-09-13) and HANDOFF-081 makes the tier a field of the package. And for the re-derivation's yield: no code defect, and four test gaps — a follower in a *subclass* of `HTTPDefaultErrorHandler` that both guards read as the default, the 3xx arm's "reads nothing" with no test behind it, the lock with none, and a subset assertion that was lazy for a missing handler and false-red under `NO_PROXY` — all closed the same day and each planted back. Forty-four defects planted across the three lanes and the session; six survived until their tests were written. The coordinator's own seventeen went 16 of 17, and the one survivor (the lock) was found independently by the review with its own. | — |
| 2026-09-13 | HANDOFF-080's rule — a box that accepts text does not store a credential the configuration declares — its option set, the decision, `cli.refuse_credential` and its callers, the notes, and their guards | Security | Four security-tier lanes attacking the design draft before code (false positives, misses and scope, guards and tests, the brief itself); the coordinating session **on the security tier** for the synthesis, the implementation and the tests; three security-tier lanes for the re-derivation, each planting its own defects in an archive of the tree | Not a downgrade. Recorded for the yield on both sides of the code. The attacking lanes changed the draft in six places before a line was written — the register at extract is compared as an *argument* and never re-read off the row, the name check runs before any sentence that spells a key, the `docker -e $TOKEN` residual holds on both sides, the note narrowed to variables whose name says what they hold, containment at twenty beside equality at eight, `headers` values joined the declared set — and the re-derivation returned NOT CLEARED three times over one commit: the rule was silently inert on Windows (the environment's keys iterated against the file's spelling), `language_tag` quoted every hosted key whole in a `403`, the name check reached the provider position only and ran after `split_key` had quoted the key, a write's own `api_key_env` declaration was not read for the name it introduced, and a tracked decision entry shipped a literal placeholder. Sixty-seven defects planted, sixteen survived until their tests were written; the coordinating session's own tests had reached none of the five defects. **The lesson for the next dispatch**, the same as HANDOFF-077's and now measured twice: a score on mutants the author designed measures the author's imagination, and the platform axis — CI runs a Windows leg, every test declared an upper-case name — is one no lane varies unless the brief names it. | 2026-09-13 |
