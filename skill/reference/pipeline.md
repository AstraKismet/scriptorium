...
| `lx extract SRC --lang L --from OLD` | carry another tracked document's translations across — what a split or renamed file needs. Holds, waivers and `origin` travel with the wording. What SRC already holds stays — a hold or waiver it lifted is not put back on wording it keeps — except a machine draft (a model's, or a memory hit) nobody held or waived; every segment where the two differ is named, and taking OLD's wording there means a person re-typing it from `lx render OLD --lang L -o -` — never copy a stored wording across yourself. `--tone` into another register drops everything SRC held, and the refusal that asks for it says how much |
...
| `lx segments SRC --lang L [--ids A,B] [--origin O] [--status S] [--limit N] [--brief] [--json]` | every stored segment in document order: id, kind, status, origin, review, waived, stranded, source, masked, target. This is how you read a translation beside its source without opening the state — `lx todo` carries no target at all, whatever `--all` does. `source` is the raw text and `masked` is what the model saw, so it is `masked` whose `⟦n⟧` line up against the target — **except where `stranded` is true**, which means the wording's `⟦n⟧` speak the numbering it was written in and the two do not correspond; re-word that segment. `--limit N` bounds the rows after the filters and `--brief` cuts each text to one line; `--json` is never truncated. A projection — it detects nothing, orders by nothing but position, exits 0 whenever it ran and 2 whenever it could not, and its `--json` is **not** a frozen contract. `lx check` is where a finding lives |
