...
- **小說這邊只做到語域這一層。** 加上 `--tone literary`，語言 brief 就從技術文件那份
  換成敘事的寫法，語域本身也進了翻譯記憶的鍵，同一句話在兩種語域下各自成立，不會
  互相蓋掉。還沒有的是 segment 周圍的東西：送進模型的仍然只有它自己的文字和區塊
  種類，前後段不在旁邊，也還沒有地方記下某個角色說話的樣子。
...
後面大致依這個順序排：把工作台的 HTTP 介面寫成有版本的 schema 定下來；EPUB，
小說多半是以這個格式流通；設定改成可以用 CLI 寫，並讓兩個階段指向
...
```bash
python -m pytest -q                # 496 passed，不碰網路
