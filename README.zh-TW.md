...
| `lx extract SRC --lang L --from OLD` | 把另一份已追蹤文件的譯文帶過來，連 hold 和 waiver 一起——拆書或改名時要用的。SRC 自己已經有的譯文照樣留著 |
...
章節動過之後再跑一次 `--from` 也沒關係。章節裡原本有的都會留下來：你改寫過的句子、
`novel.md` 從來沒翻過的段落、你解除過的 hold，都不會被換掉。`novel.md` 的譯文只會
進到兩種位置：章節那一段還是空的，或者那一段只有一份模型打的草稿，而且沒人 hold、
也沒人 waive 過。兩邊還有出入的段落會一段一段列出來，被換掉的草稿也會。如果這次是
`novel.md` 那邊寫得比較好——你拆完書又回頭改了原本那一份——就用
`lx render novel.md --lang zh-TW -o -` 看它怎麼寫，再到章節裡重打一次。狀態裡分不
出「章節後來改過」和「原書後來改過」，所以指令不替你猜。
...
因為還有一章沒 extract。只要 `lx extract <章節> --from novel.md` 能解決其中幾段，
它就會建議你跑：carry 會留下章節原本有的東西，所以同一個語域的章節都算，建議裡也會
點名哪幾份模型草稿會被換掉。章節要是在另一個語域，carry 非加不可的 `--tone` 會讓它
原有的譯文一句也留不下來，這時它反而叫你不要 carry。真的要刪就加
`--discard-wording`，丟掉的正好是拒絕訊息點名的那幾段。
...
```bash
python -m pytest -q                # 2390 collected，不碰網路
