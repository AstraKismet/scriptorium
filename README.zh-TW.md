...
還有 `config/dnt.txt` 裡的每一個術語，都換成 `⟦n⟧` placeholder；原文自己寫出來的
`⟦n⟧` 也算一份，而且排在最前面遮——模型看到的每一個 placeholder，才保證是 pipeline
自己放進去的那一個。區塊語法——標題、清單符號、引用區塊的標記、表格的 `|`——
雖然不遮罩，但從來沒有離開過骨架，所以模型一樣看不到。要支援新的行內語法，
做法是在 `mask.py` 裡加一條 pattern，而不是在 prompt 裡多寫一句話。
...
CI 上有一組語料庫在把關，裡面收了 58 份刻意刁難的輸入（Markdown 36 份、純文字
22 份），Linux 與 Windows 都跑，從磁碟上的位元組一路驗到寫回去的位元組。
...
傳輸層不跟隨任何重導向：後端若回 3xx，第一跳就拒絕，內文與 `Location` 都不讀，
所以金鑰只會送到 `base_url` 指定的位址。
...
是原樣送上線的東西，命令列根本不給寫。不管是哪個欄位，拒絕訊息都不會把被拒的值再講一次；
欄位裡放著它根本不可能合法持有的值時——這個版本沒有的 `kind`、不是變數名稱的
`api_key_env`、不是 http(s) 位址的 `base_url`——`lx providers` 和 `lx config get`
也不會把它顯示出來。設定檔自己宣告的金鑰，任何欄位都存不進去：值只要等於某個
`api_key_env` 指名的環境變數內容（或 `headers` 的值、`base_url` 的 userinfo），
不管貼進 `model`、routing 的 model、`tone`、新後端的名字、`lx extract --tone`
還是 `lx glossary set`，一律拒絕，拒絕句只點名變數、不重複值。比對的範圍就只有
這些，所以後端真的在服務的 model id 永遠不會被擋；金鑰若 export 在一個還沒被
任何 `api_key_env` 指名的變數底下，會照存，但只要變數名看得出裝的是金鑰
（`GROQ_API_KEY`，而不是 `GW`），就會多印一行 note 告訴你它是哪個變數的內容。
另外沒有任何 `lx` 指令會在命令列上收金鑰——
...
的另一份已追蹤文件裡一字不差地寫出來——比的是 `lx render` 會寫出的樣子，所以之後
...
原封不動。這也不是抹除：SQLite 之後會重複利用那塊空間，卻不會先把舊內容蓋掉，所以
...
![審校工作台：英文原文與正體中文譯文並排，驗證失敗寫在旁邊的空白處，右欄是翻譯記憶裡
一句幾乎相同的來源留下的措辭](docs/workbench.webp)

原文與譯文並排，placeholder 高亮，驗證失敗會以旁註標在對應的 segment 邊上。
翻譯、潤稿、修復、檢查、閱讀、寫檔、提交都在工具列上。編輯欄位失去焦點就自動存檔，
內容在收回時會先正規化，包含把模型弄壞的 placeholder 括號修回來。segment 清單是
虛擬化的，所以一章和一本五千段的小說捲起來是一樣的。

**旁欄，回答兩個以前問不到的問題。**在你正在看的那一段旁邊：模型到底被告知了這本書
該是什麼聲音——那份組裝就是請求真正帶出去的那一份，不是另外拼一次的——以及記憶裡
*幾乎*就是這一句的來源留下的措辭。記憶只答完全相同的句子，不然就不答；而第二章寫
*She had not slept*、第一章寫的是 *She had not slept well*，這種時候它什麼都給不了。
近似結果只供參考，而且刻意沒有套用按鈕：近似命中在定義上就會在 placeholder 集合上
有出入，把一段的措辭搬到另一段，渲染出來就是一個裸的 `⟦2⟧`。看上眼就自己重打一次，
那才是你的字。

**Hold 與 waive，逐段。**「Hold」讓所有會挑工作的佇列——初稿、修復、潤稿——都避開
你還在想的那一段，同時那一段照樣可以編輯，而明確點名它仍然送得到。「Waive」用來回答
一條你已經讀過、並且決定就這樣的規則：規則名稱與訊息都留著，只是從 error 降成 warn，
不再擋住 build；而它被釘在你當時讀的那一份措辭上，字一改，豁免就掉。

**閱讀是一個頁面，不是一個對話框。**「Read」把整章按渲染後的樣子攤開——連續的散文、
適合閱讀的行寬，未翻譯的段落有標記，驗證失敗的那幾段在左緣拉一條紅線。打開時停在你在
ledger 裡正看著的那一段；在這裡點一段是移到那一段，雙擊或按 Enter 才跳回 ledger 的那一段，
按「Back to the ledger」則回到你正在讀的段落。段落編號寫在網址裡，所以重新整理也一樣。
句子邊界由 Python 決定，你在這裡看到的斷句，和 `lx sentences`、和 CI 看到的是同一條規則。
...
設定好的模型。*Backends* 用來新增或編輯一份設定：`kind`、`base_url`、`model`、
`api_key_env`、`timeout`、`temperature`、`max_tokens`、`retries`。本機跑的、
區網另一台機器上的、雲端 API，三種都能從這個表單接起來。

*Routing* 是**另一個畫面**，這個拆分不是為了好看。一份後端設定裡有什麼，和哪個階段
該用哪一份，是兩個不同的問題，寫入的語意也不同——前者是對一個具名 provider 逐欄寫入，
後者是三個互不相干的鍵——而把兩者混在同一張表單裡的那個版本，你改三個階段它只存了
第一個。
...
金鑰的值在它和 `base_url` 上都會被擋下來；頁面上的拒絕訊息也不會把被拒的值再講一次，
不管它是打在哪一格。*模型*那一格、*Name* 那一格和其他每一格，
只要值是設定檔已經宣告的金鑰——旁邊那個變數名指到的內容——就會被拒；
金鑰若 export 在還沒被指名、但名字看得出是金鑰的變數底下，會存進去並附一行 note 說明是哪個變數。改 `base_url` 要另外勾一次確認，因為那決定了稿子
和金鑰被送去哪裡。後端不能從瀏覽器刪除，那是 `lx config unset` 或直接改檔案的事。

它只是一層外殼，底下呼叫的和 CLI 是同一批函式，不存在第二套會走鐘的實作。它的原始碼是
`studio/web/` 底下的一個 React 應用，而**建置產物是進版控的**，直接放進 Python 套件裡，
所以在一份沒裝 Node 的乾淨 checkout 上 `lx web` 照樣能跑——這和「管線不依賴任何需要編譯
的東西」是同一條要求。它啟動時會讀契約版本，遇到不認得的數字就直接拒絕啟動，而不是對著
一個已經在腳下換過的介面半推半就地運作。
...
```bash
python -m pytest -q                # 2950 collected，不碰網路
