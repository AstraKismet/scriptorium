...
還有 `novel.md` 沒翻、你在章節裡翻了的段落，都一樣；你在這些段落上解除的 hold，也
不會被 `novel.md` 那一份又掛回去。只有兩種情況，`novel.md` 的譯文才會填進去：章節
...
不同的章節，要用 `--from` 就得加 `--tone`，而一加 `--tone`，章節自己存的譯文就用
不上了，落進去的全是 `novel.md` 那一份。所以這種章節只有在除了機器草稿之外沒有別
的會被換掉時，它才會建議，而且指令裡會帶上 `--tone`；否則它就反過來叫你別用
`--from`。真的要刪就加 `--discard-wording`，丟掉的正好是拒絕訊息點名的那幾段。
...
```bash
python -m pytest -q                # 2426 collected，不碰網路
