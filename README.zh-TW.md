...
**沒有任何欄位收 API key**：`api_key_env` 收的是環境變數的*名字*，長得像金鑰的值
會被擋下來，而且不會被覆述一次。改 `base_url` 要另外勾一次確認，因為那決定了稿子
...
設定從 CLI 和瀏覽器都寫得動，而且兩邊都能讓每個階段指向不同的後端和模型。
...
```bash
python -m pytest -q                # 1700 tests，不碰網路
