...
任何 `api_key_env` 指名的變數底下，會照存，但多印一行 note 告訴你它是哪個變數的內容。
...
金鑰若 export 在還沒被指名的變數底下，會存進去並附一行 note 說明是哪個變數。改 `base_url` 要另外勾一次確認，因為那決定了稿子
...
```bash
python -m pytest -q                # 2869 collected，不碰網路
